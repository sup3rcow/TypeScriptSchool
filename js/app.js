...
var _ = require("lodash"); //ovo je malo drugacije, jer importas kroz rederencce path?
function GetAllBooks() {
    var books = [
        { id: 1, title: 'Ulysses1', author: 'James mr', available: true, category: enums_1.Category.Fiction },
        { id: 2, title: 'Ulysses2', author: 'James jr', available: false, category: enums_1.Category.Fiction },
        { id: 3, title: 'Ulysses3', author: 'James ms', available: true, category: enums_1.Category.Poetry },
        { id: 4, title: 'Ulysses4', author: 'James js', available: true, category: enums_1.Category.Fiction }
    ];
    return books;
}
...
function getBookTitlesByyCategory(categoryFilter, books) {
    if (categoryFilter === void 0) { categoryFilter = enums_1.Category.Fiction; }
    console.log('Traže se knjige kategorije: ' + categoryFilter + ':' + enums_1.Category[categoryFilter]);
    var filteredTitles = [];
...
function LogBookTitles() {
    ...
    var fictionBooks = getBookTitlesByyCategory(enums_1.Category.Fiction, allBooks);
    // for(let title of fictionBooks){
    //     console.log('Knjiga for: ' + title);
    //}
    fictionBooks.forEach(function (title) { console.log('Knjiga foreach: ' + title); });
}
function GetBookById(id) {
    var allBooks = GetAllBooks();
    return allBooks.filter(function (book) { return book.id === id && 1 === 1; })[0]; //[0] posto se ocekuje da funkcija nadje samo jednu knjigu po id-u, odmah izvuces
    //jedinog clana iz array-a i imas objekt book koji je inace annonymus jer jos nismo dosli do klasa sa učenjem
}
function CreateCustommerID(name, id) {
    return name + id;
}
...
//lodash
var snakeCaseTitle = _.snakeCase('Teskt za testiranje');
